...
public class CustomSizeDialog extends Dialog {
	...
	public CustomSizeDialog(Context context, CustomSizeDialogViewGroup.OnGroupItemClickListener listener,
	                        GoodsEntitys.DataBeanX mData, List<GoodsSaleEntity.GoodsSaleData> mSale, String orderId) {
	...
	private void setCustomDialog() {
		...
		ivVip = (ImageView) mView.findViewById(R.id.iv_vip);
...
//		}
		...
		if (mData.getData().getIsVip() == 1) {
			if (null == orderId) {
				btnCart.setEnabled(false);
				btnCart.setBackground(context.getResources().getDrawable(R.drawable.shape_corner_solid));
			} else {
				btnCart.setEnabled(true);
				btnCart.setBackground(context.getResources().getDrawable(R.drawable.shape_corner_five));
			}
			btnCart.setText("立即兑换");
			ivVip.setVisibility(View.VISIBLE);
		} else if (mData.getData().getIsVip() == 0) {
			btnCart.setText("加入购物车");
