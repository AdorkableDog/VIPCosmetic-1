...
public class GoodsDetailActivity extends BaseActivity implements CustomRollPagerView.ScrollChangeListener, GoodsDetailContentAdapter.ShowDetialInterface,
	...
	private String productId, sproductId, orderId;
...
//				}
	...
	public void showDetial() {
		...
		if (dialog == null) {
			dialog = new CustomSizeDialog(GoodsDetailActivity.this, GoodsDetailActivity.this, data, mSale, orderId);
		}
		if (!dialog.isShowing()) {
			dialog.show();
		}
			...
			public void onClick(View view) {
				if (mUtils.isLogin()) {
					if (checkForGoods()) {
						...
						if (data.getData().getIsVip() == 1) {
							if (null == orderId) {
								ToastUtils.showToast(GoodsDetailActivity.this, "无法兑换");
							} else {
						...
						} else if (data.getData().getIsVip() == 0) {
...
//					}
					...
					if (data.getData().getIsVip() == 1) {
						if (null == orderId) {
							...
							btnAdd.setBackground(getResources().getDrawable(R.drawable.shape_corner_solid));
						} else {
							btnAdd.setEnabled(true);
							btnAdd.setBackground(getResources().getDrawable(R.drawable.shape_corner_five));
						}
						btnAdd.setText("立即兑换");
					} else {
						btnAdd.setText("加入购物车");
					}
				...
				public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {
					switch (i) {
						...
						case 2:
							...
							ClipData mClipData = ClipData.newPlainText("url", URLBuilder.getUrl(shareData.getUrl()));
							cm.setPrimaryClip(mClipData);
							//+"\n这是我的邀请码,注册时一定要填写哦^_^"
							ToastUtils.showToast(GoodsDetailActivity.this, "复制成功,可以发给朋友们了^_^");
							break;
					}
					dismissDialog2();
				}
