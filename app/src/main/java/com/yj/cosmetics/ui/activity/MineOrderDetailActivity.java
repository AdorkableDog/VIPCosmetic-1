...
//	TextView tvGoodsSum;
	...
	@BindView(R.id.order_detial_rl_money)
	RelativeLayout rlDetialMoney;
	@BindView(R.id.order_detial_money)
	TextView tvDetialMoney;
	...
	MineOrderListGoodsDetailAdapter itemsAdapter;
	...
	private List<OrderDetailEntity.OrderDetialData.OrderDetialItem> mItemsList;
...
//                    disMissDialog();
	...
	protected void initView() {
		...
		itemsAdapter = new MineOrderListGoodsDetailAdapter(this, mItemsList, oid, flag);
	...
	@OnClick({R.id.order_detial_rl_logistic, R.id.order_detial_pay, R.id.order_detial_delete, R.id.title_tv_next, R.id.tv_vip_goods})
...
//					if (!TextUtils.isEmpty(data.getOrderPayStyle())) {}
			...
			case R.id.title_tv_next:
				doAsyncGetShare(data.getShopOrderId(), data.getShopId());
				break;
			case R.id.tv_vip_goods:
				Intent intentac = new Intent(this, HomeGoodsListActivity.class);
				intentac.putExtra("orderId", data.getShopOrderId());
				intentac.putExtra("isVip", "1");
				startActivity(intentac);
				break;
		}
	}

	private void showPayDialog(final String orderId, String[] payMode, Integer[] payIcon, String tvPrice) {
	...
	private void doAsyncGetData() {
		...
		OkHttpUtils.post().url(URLBuilder.URLBaseHeader + "/phone/user/orderDetail")
			...
			public OrderDetailEntity parseNetworkResponse(Response response) throws Exception {
				...
				LogUtils.e("OrderDetailEntity ---json的值" + json);
		...
		switch (data.getOrderState()) {
			case "0":
				if (data.getProductState().equals("2")) {
					...
					tvRight.setVisibility(View.GONE);
					tvVipGoods.setVisibility(View.GONE);
					tvLeft.setText("删除订单");
...
//				ivState.setImageResource(R.mipmap.mark_fill_z);
		...
		if (data.getIsAccelerate() != null && data.getIsAccelerate().equals("1")) {
			if (data.getItems().get(0).getIsVip().equals("0")) {
				if (null != data.getSuperiorOrderName() && data.getSuperiorOrderName().equals("0")) {
					rlTitle.setVisibility(View.VISIBLE);
					tvVipGoods.setVisibility(View.VISIBLE);
		...
		if (data.getOrderPayStyle().equals("0")) {
			tvPayStyle.setText("支付宝");
		} else if (data.getOrderPayStyle().equals("1")) {
		...
		tvTotal.setText("￥" + data.getPayMoney());
...
//		tvGoodsSum.setText(data.getOrderMoney());
		if (null != data.getDeductionMoney() && !data.getDeductionMoney().equals("0.00")) {
			rlPrice.setVisibility(View.VISIBLE);
			rlDetialMoney.setVisibility(View.VISIBLE);
			tvGoodsPrice.setText("￥" + data.getOrderPaymoney());
			tvDetialMoney.setText("-￥" + data.getDeductionMoney());
		} else {
			rlPrice.setVisibility(View.GONE);
			rlDetialMoney.setVisibility(View.GONE);
		}
		...
		if (null != data.getItemsList()) {
			if (null != data.getExchangeOrderNum()) {
				tvOrderNumber.setVisibility(View.VISIBLE);
				tvOrderNumber.setText("订单号：" + data.getExchangeOrderNum());
				mItemsList.clear();
				mItemsList.addAll(data.getItemsList());
				itemsAdapter.setOrderState(data.getOrderState());
				itemsAdapter.setProductState(data.getProductState());
				itemsAdapter.setRefundType(data.getRefundType());
				//getOrderMoney 总价  getOrderSendCosts 运费  //getOrderBalance 余额
				itemsAdapter.setOrderRefundPic(data.getOrderPaymoney(), data.getOrderSendCosts(), data.getOrderBalance());
		...
		if (shareData != null && shareData.getUrl() != null) {
			...
			if (shareData.getTitle() != null) {
				if (platform.equals(SinaWeibo.NAME)) {
					...
					oks.setText(URLBuilder.getUrl(shareData.getUrl()));
