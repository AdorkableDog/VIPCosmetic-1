...
//						newVersion = data.getAppVersion();
	...
	private void showUpdateDialog() {
		...
		updateDialog.getSize().setText(readableFileSize(Long.parseLong(data.getAppLen())));

		if (data.getAppForce().equals("1")) {// 0: 是 1： 否
			//不是强制更新
			updateDialog.setIsCancelable(true);
			updateDialog.getForce().setOnClickListener(
...
//					isStartServices();
			...
			//强制更新
			updateDialog.getClose().setVisibility(View.GONE);
...
//			String urLs = URLBuilder.getURLs(data.getAppLink());
			intent.putExtra("url", URLBuilder.getUrl(data.getAppLink()));
			...
			alertDialog = new AlertDialog.Builder(SplashActivity.this)
					...
					.setPositiveButton("确认", new DialogInterface.OnClickListener() {
						...
						public void onClick(DialogInterface dialog, int which) {
							Intent intent = new Intent(SplashActivity.this, DownAPKService.class);
//							mUtils.saveLink(URLBuilder.URLBaseHeader + URLBuilder.getURLs(data.getAppLink()));
//							Log.e(TAG, "getAppLink: " +  URLBuilder.getURLs(data.getAppLink()));
							String urLs = URLBuilder.getUrl(data.getAppLink());
