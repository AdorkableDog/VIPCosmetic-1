...
import com.yj.cosmetics.model.MoneyEntity;
...
public class SettlementGoodsActivity extends BaseActivity {
	...
	private String proId, proNumber, pro, addressId, tickPic, sproductId, shopId, orderId;
...
//					tvTotal.setText("0.00");
	...
	private void setData(SettlementGoodsEntity.SettlementGoodsData data) {
		...
		if (null == orderId) {
			...
			if (!TextUtils.isEmpty(data.getDeductionMoney())) {
				tvDeduction.setText("-￥" + data.getDeductionMoney());
					...
					if (response != null && response.getCode().equals("200")) {
						if (response.getData() != null && response.getData().getAppPayJson() != null && !TextUtils.isEmpty(response.getData().getAppPayJson().getAppid())) {
							...
							if (total - rest <= 0) {
								...
								Intent intent = new Intent(SettlementGoodsActivity.this, MineOrderDetailActivity.class);
								Log.e("-0-", "==" + response.getData().getOrderId());
...
//					.addParams("key", RSAUtils.encryptByPublicKey(Constant.KEY))
	...
	private void payWithMoney(String pay) {
		Map<String, String> map = new HashMap<>();
		map.put("userId", mUtils.getUid());
		map.put("proId", proId);
		map.put("pro", pro);
...
//					.addParams("key", RSAUtils.encryptByPublicKey(Constant.KEY))
					...
					.tag(this).build().execute(new Utils.MyResultCallback<MoneyEntity>() {
				@Override
				public void onBefore(Request request) {
					super.onBefore(request);
					if (mDialog == null) {
				...
				@Override
				public MoneyEntity parseNetworkResponse(Response response) throws Exception {
					String json = response.body().string().trim();
					LogUtils.i("json的值" + json);
					return new Gson().fromJson(json, MoneyEntity.class);
				}

				@Override
				public void onResponse(MoneyEntity response) {
					if (response != null && response.getCode().equals("200")) {
						if (response.getData().getOrderMoneyAll() == 0) {
							ToastUtils.showToast(SettlementGoodsActivity.this, "支付成功");
							Intent intent = new Intent(SettlementGoodsActivity.this, PayResultActivity.class);
							mUtils.savePayOrder(response.getData().getOrderId());
							mUtils.savePayType("goods");
