...
public class MyAccListAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
	...
	public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, final int position) {
		if (holder instanceof PartakeViewHolder) {
			Glide.with(mContext)
					.load(URLBuilder.getUrl(data.get(position).getListImg()))
					.asBitmap()
					.centerCrop()
					.error(R.mipmap.default_avatar)
					.into(((PartakeViewHolder) holder).ivStore);
			((PartakeViewHolder) holder).tvDateTime.setText(data.get(position).getInsertTime());
			((PartakeViewHolder) holder).shopName.setText("订单:" + data.get(position).getOrderNum());
			if (falg == 0) {
				...
				((PartakeViewHolder) holder).ivSuperScript.setVisibility(View.GONE);
				((PartakeViewHolder) holder).tvConsume.setVisibility(View.VISIBLE);
				if (data.get(position).getRowNum() > 9) {
					((PartakeViewHolder) holder).tvConsume.setText("排队:" + String.valueOf(data.get(position).getRowNum()));
				} else {
					((PartakeViewHolder) holder).tvConsume.setText("排队：0" + data.get(position).getRowNum());
				}
				((PartakeViewHolder) holder).tvSpeed.setVisibility(View.VISIBLE);
				if (data.get(position).getSuperiorOrderName() == 0) {
					((PartakeViewHolder) holder).riImg.setVisibility(View.VISIBLE);
					((PartakeViewHolder) holder).tvImg.setVisibility(View.VISIBLE);
				} else {
					((PartakeViewHolder) holder).riImg.setVisibility(View.GONE);
					((PartakeViewHolder) holder).tvImg.setVisibility(View.GONE);
				}
				...
				((PartakeViewHolder) holder).ivSuperScript.setVisibility(View.VISIBLE);
				...
				((PartakeViewHolder) holder).tvSpeed.setVisibility(View.GONE);
				((PartakeViewHolder) holder).riImg.setVisibility(View.GONE);
				((PartakeViewHolder) holder).tvImg.setVisibility(View.GONE);
			}
...
//		TextView pic;//价格
		...
		public void onClick(View v) {
			if (mListener != null) {
				...
				} else if (v.getId() == R.id.ri_img) {
					mListener.onVIPClick(v, getPosition());
				}
			}
		}
	}
	...
	public interface onItemClickListener {
		...
		void onVIPClick(View view, int postion);
	}
}
