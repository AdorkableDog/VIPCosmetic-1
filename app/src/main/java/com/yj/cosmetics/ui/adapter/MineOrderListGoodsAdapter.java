...
public class MineOrderListGoodsAdapter extends BaseAdapter {
	...
	public MineOrderListGoodsAdapter(Context context, List<MineOrderEntity.DataBean.UserOrderMapsBean.UserOrderdetailMapsBean> data, String name) {
		this.context = context;
		this.data = data;
		this.name = name;
		layoutInflater = LayoutInflater.from(context);
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (layoutInflater != null) {
			...
			ImageView ivVip = (ImageView) view.findViewById(R.id.iv_vip);
			ImageView ivExchange = (ImageView) view.findViewById(R.id.iv_exchange);
			if (data.get(position).getIsVip().equals("0")) {
				ivVip.setVisibility(View.GONE);
			} else if (data.get(position).getIsVip().equals("1")) {
